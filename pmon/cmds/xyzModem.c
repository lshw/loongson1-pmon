...
#define NAK 0x15
#define CAN 0x18
#define XYZ_EOF 0x1A		/* ^Z for DOS officionados */
...
#define tstc tgt_testchar
#define udelay delay
#define getc tgt_getchar
#define putc tgt_putchar
#define NULL 0
		  ...
		  if (xyz.blk == xyz.next_blk)
		      ...
		      if (1)
			{
			  ...
			  if ((xyz.bufp[xyz.len - 1] == XYZ_EOF) &&
			      (xyz.bufp[xyz.len - 2] == XYZ_EOF) &&
			      (xyz.bufp[xyz.len - 3] == XYZ_EOF))
			    {
			      while (xyz.len
				     && (xyz.bufp[xyz.len - 1] == XYZ_EOF))
				{
				  xyz.len--;
				}
			    }
