...
#else
	#define PAGE_TO_FLASH(x)  ((x)<<11)
	#define PAGE_TO_FLASH_H(x)  ((x)>>1)

	#define PAGE_TO_FLASH_S(x)  ((x)<<12)
	#define PAGE_TO_FLASH_H_S(x)  ((x))
#endif
...
//    if(ask == (VT_TO_PHY(ram)+((len+3)/4)*4)){break;}
    nand_udelay(60);
    }
    while(!rdy_status())
        nand_udelay(20);
...
void nand_test_new()
{
	...
	_WL(0xbfd00420, 0x14000002);	//MUX the PWM & SPI pin to nandflash function
	_WL(0xbfd010c0, 0x0);		//set all pin to their function
	_WL(0xbfd010c4, 0x0);
	_WL(0xbfd010c8, 0x0);
...
static const Cmd Cmds[] =
{
	...
	{"nand_test_new"," ",0,"NANDFlash write data to first page and then read back to check",nand_test_new,0,99,CMD_REPEAT},
	{0, 0}
};
