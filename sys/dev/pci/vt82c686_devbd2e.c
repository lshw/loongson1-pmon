...
static void myfixup()
{
	...
	//_pci_conf_writen(pdev, 0x47, 0x20,1); 
	...
	/* route PIRQA-D irq */
	_pci_conf_writen(pdev,0x55, 0x90,1); /* bit 7-4, PIRQA */
	_pci_conf_writen(pdev,0x56, 0xba,1); /* bit 7-4, PIRQC; 3-0, PIRQB */
	_pci_conf_writen(pdev,0x57, 0xd0,1); /* bit 7-4, PIRQD */

	/* enable function 5/6, audio/modem */
	_pci_conf_readn(pdev,0x85, &c,1); 
	c &= ~(0x3<<2);
	_pci_conf_writen(pdev,0x85,c,1);

//fixup fuction 1

    pdev=_pci_make_tag(0,17,1);
