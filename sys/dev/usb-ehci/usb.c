...
extern void delay_us(microseconds);
...
struct usb_device * usb_alloc_new_device(void *hc_private)
{
	...
	dev_index++;
	usb_dev[dev_index-1].hc_private = hc_private; //lxy
...
//	printf ("\n");
	...
	config_found(dev->hc_private, dev, usbprint);		//lxy
