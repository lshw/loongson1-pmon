...
static unsigned long usb_stor_read(int device, unsigned long blknr, unsigned long blkcnt, 
										unsigned long *buffer, unsigned char *cp);
...
int usb_stor_info(void)
{
	...
	if (usb_max_devs > 0) {
		for (i = 0; i < usb_max_devs; i++) {
			printf("  Device %d: ", i);
//			dev_print(&usb_dev_desc[i]);		//lxy disable it
		}
		return 0;
	}
 ...
 */
extern	void flush_invalidate(u32 addr, int size, int flush);
int usb_stor_scan(int mode)
{
	...
	flush_invalidate(&usb_ccb, sizeof(usb_ccb), 1);		//lxy
	usb_ccb_p = (ccb*)CACHED_TO_UNCACHED(&usb_ccb);		//lxy
	/* GJ */
...
static unsigned long usb_stor_read(int device, unsigned long blknr, unsigned long blkcnt, unsigned long *buffer, unsigned char *cp)
...
//	printf ("Read Capacity returns: 0x%lx, 0x%lx\n", cap[0], cap[1]);
	...
	cap[0] = ((unsigned long)(
		(((unsigned long)(cap[0]) & (unsigned int)0x000000ffUL) << 24) |
		(((unsigned long)(cap[0]) & (unsigned int)0x0000ff00UL) <<  8) |
		(((unsigned long)(cap[0]) & (unsigned int)0x00ff0000UL) >>  8) |
		(((unsigned long)(cap[0]) & (unsigned int)0xff000000UL) >> 24) ));
	cap[1] = ((unsigned long)(
		(((unsigned long)(cap[1]) & (unsigned int)0x000000ffUL) << 24) |
		(((unsigned long)(cap[1]) & (unsigned int)0x0000ff00UL) <<  8) |
		(((unsigned long)(cap[1]) & (unsigned int)0x00ff0000UL) >>  8) |
		(((unsigned long)(cap[1]) & (unsigned int)0xff000000UL) >> 24) ));
#else
...
//	cap[0] = cpu_to_le32(cap[0]);
//	cap[1] = cpu_to_le32(cap[1]);
...
/*************************************************************************************************************/		//lxy
void storage_free(struct usb_device *dev)
{
	struct device *self = dev->match;
	struct cfdata *dv_cfdata = self->dv_cfdata;
	struct cfdriver *cd = dv_cfdata->cf_driver;

	TAILQ_REMOVE(&alldevs, self, dv_list);
	...
	dv_cfdata->cf_fstate = FSTATE_NOTFOUND;

}

static int usb_match(struct device *parent, void *match, void *aux)
{
	struct usb_device *dev = aux;

	pci_sync_cache(0, usb_ccb, sizeof(usb_ccb), 1);
	usb_ccb_p = (ccb*)CACHED_TO_UNCACHED(&usb_ccb);
#if 0
	if (usb_stor_scan(1))
	{
		printf ("probe usb_disk error!\n");
		return 0;
	}
	else
	{
		printf ("probe usb_disk sucessfully!\n");
		return 1;
	}
#else
	if(usb_max_devs==USB_MAX_STOR_DEV) {
	...
	if(usb_storage_probe(dev,0,&usb_stor[usb_max_devs])) { /* ok, storage devices */
...
static void usb_attach(struct device *parent, struct device *self, void *aux)
{
	struct usb_device * dev = aux;
	dev->match = self;
...
void usb_strategy(struct buf *bp)
{
	...
	device = dev & 0xff;
	blkno = bp->b_blkno;
	d_secsize = usb_dev_desc[device].blksz;
	...
	blkno = blkno /(d_secsize/DEV_BSIZE);
