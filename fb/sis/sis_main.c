...
struct pci_dev sisvga_dev;

struct pci_dev *
pci_get_device(unsigned int vendor, unsigned int device, struct pci_dev *from)
{
if(sisvga_dev.vendor == vendor && sisvga_dev.device == device)return &sisvga_dev;
else return 0;
}
...
void __iomem *pci_map_rom(struct pci_dev *pdev, size_t *size)
{
	...
	long addr,len;
        ...
        addr=pci_conf_read(0,pdev->pa.pa_tag,PCI_ROM_ADDRESS);
		pci_conf_write(0,pdev->pa.pa_tag,PCI_ROM_ADDRESS,-1);
		len=pci_conf_read(0,pdev->pa.pa_tag,PCI_ROM_ADDRESS);
		pci_conf_write(0,pdev->pa.pa_tag,PCI_ROM_ADDRESS,addr);
		...
		if(len)
		{
		 pci_conf_write(0,pdev->pa.pa_tag,PCI_ROM_ADDRESS,addr|1);
		rom=(void *)((addr&~0xf)|0xb0000000);
		}
		else
		rom=0;
		*size=len;
...
#define pci_unmap_rom(...)
#define vmalloc malloc
#define vfree free
#define kmalloc(s,...) malloc(s)
#define kfree free
#define fb_dealloc_cmap(...)
...
sisfb_poh_new_node(struct SIS_HEAP *memheap)
{
	...
	if(memheap->poh_freelist == NULL) {
		poha = (void *)kmalloc(SIS_OH_ALLOC_SIZE, GFP_KERNEL);
		if(!poha)
			return NULL;
			...
			unsigned int enda, unsigned int mapsize)
{
	...
	if(readl((long)ivideo->video_vbase) != 0)
		return 0;

	for(i = starta; i <= enda; i++) {
		pos = 1 << i;
		if(pos < mapsize) {
			if(readl((long)ivideo->video_vbase + pos) != pos)
				return 0;
		} else
			return 0;
	}
...
{
struct fb_copyarea r={.sx=sx,.sy=sy,.dx=dx,.dy=dy,.width=width,.height=height};

sisfb->fbops->fb_copyarea(sisfb,&r);
}
...
printf(" ivideo->SiS_Pr.SiS_P3c4=%x\n", ivideo->SiS_Pr.SiS_P3c4);
	outSISIDXREG(SISSR, 0x05, 0x86);
...
#if 1
static void vgadev_init(struct pci_dev *pcidev,struct pci_attach_args *pa)
{
    unsigned short  vendor;
    unsigned short  device,class;
    unsigned short  subsystem_vendor;
	...
	pcidev->subsystem_vendor=subsystem_vendor;
    pcidev->subsystem_device=subsystem_device;
	pcidev->pa=*pa;
}
...
{
	char *options = NULL;
    struct pci_device_id *id=pci_match_device(sisfb_pci_table,&vga_dev->pa);
//	sisfb_setup(options);
if(id)
{
vgadev_init(&sisvga_dev,&vga_dev->pa);
return sisfb_probe(&sisvga_dev,id);
}
return 0;
}
#endif
...
int __init sisfb_init_module(void)
{
	...
	sisfb_init();
	return fb_screen_base;
}

#if 0
struct cfattach sisfb_ca = {
	sizeof(struct net_device), sisfb_match, sisfb_attach
};
